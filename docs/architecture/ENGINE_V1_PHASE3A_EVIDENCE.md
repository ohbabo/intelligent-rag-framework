...
Companion to `ENGINE_V1_PHASE3A_ARCHITECTURE_DECISION.md`. Sparse per-method
self-call / store read-write table measured by AST over `ragcore/engine.py`
(current main), plus the derived values the ADR relies on. Operation legend:
`R` read · `W` whole-replace · `A` append/insert-into · `D` delete-from ·
`M` merge/diff-update · `I` revision-identity · `ID` id-allocation. Store names
are shown without the leading underscore. `__init__` (construction) and the
bulk `_install`/`_state_view` restore/encode boundary are listed but excluded
from *operational* mutation-ownership totals.
...
| `_install` | priv | C10 | — | claim_gap_refs:W claim_lifecycle_events:W claims:W contradictions:W entities:W evidences:W gap_dedup_index:W gap_resolutions:W gaps:W hint_evidence_types:W lifecycle_seq:W next_id:ID observations:W relations:W resolved_contradictions:W rule_definitions:W rule_stats:W | — | — | mutating |
...
| `add_claim` | pub  | C2 | claims entities | claims:A | _advance_state_revision _allocate_id | C1:_advance_state_revision C1:_allocate_id | mutating |
| `add_entity` | pub  | C2 | entities | entities:A | _advance_state_revision _allocate_id | C1:_advance_state_revision C1:_allocate_id | mutating |
| `add_evidence` | pub  | C2 | evidences | evidences:A | _advance_state_revision _allocate_id _assert_claim_exists | C1:_advance_state_revision C1:_allocate_id C1:_assert_claim_exists | mutating |
| `add_observation` | pub  | C2 | observations | observations:A | _advance_state_revision _allocate_id _assert_entity_exists | C1:_advance_state_revision C1:_allocate_id C1:_assert_entity_exists | mutating |
...
| `add_relation` | pub  | C3 | relations | relations:A | _advance_state_revision _allocate_id _id_exists | C1:_advance_state_revision C1:_allocate_id C1:_id_exists | mutating |
| `get_relation` | pub  | C3 | relations | — | — | — | read-only |
| `add_gap` | pub  | C4 | claim_gap_refs claims gap_dedup_index gaps | claim_gap_refs:A gap_dedup_index:A gaps:A | _advance_state_revision _allocate_id _assert_claim_exists | C1:_advance_state_revision C1:_allocate_id C1:_assert_claim_exists | mutating |
...
| `confirm_claim_if_ready` | pub  | C5 | claims | claims:A | _advance_state_revision _assert_claim_exists _record_claim_lifecycle_transition gap_resolution gaps_for_claim | C1:_advance_state_revision C1:_assert_claim_exists C4:gap_resolution C4:gaps_for_claim C6:_record_claim_lifecycle_transition | mutating |
| `contradictions_for_claim` | pub  | C5 | contradictions | — | _assert_claim_exists | C1:_assert_claim_exists | read-only |
| `dispute_claim_if_ready` | pub  | C5 | claims contradictions | claims:A | _advance_state_revision _assert_claim_exists _record_claim_lifecycle_transition | C1:_advance_state_revision C1:_assert_claim_exists C6:_record_claim_lifecycle_transition | mutating |
| `refute_claim_if_ready` | pub  | C5 | claims contradictions | claims:A | _advance_state_revision _assert_claim_exists _record_claim_lifecycle_transition | C1:_advance_state_revision C1:_assert_claim_exists C6:_record_claim_lifecycle_transition | mutating |
| `refute_disputed_claim_if_ready` | pub  | C5 | claims evidences | claims:A | _advance_state_revision _assert_claim_exists _record_claim_lifecycle_transition active_contradictions_for_claim | C1:_advance_state_revision C1:_assert_claim_exists C6:_record_claim_lifecycle_transition | mutating |
| `refute_disputed_claim_if_ready_by_freshness` | pub  | C5 | claims evidences | claims:A | _advance_state_revision _assert_claim_exists _record_claim_lifecycle_transition active_contradictions_by_freshness | C1:_advance_state_revision C1:_assert_claim_exists C6:_record_claim_lifecycle_transition | mutating |
| `register_contradiction` | pub  | C5 | contradictions | contradictions:A | _advance_state_revision _assert_claim_exists _assert_evidence_exists | C1:_advance_state_revision C1:_assert_claim_exists C1:_assert_evidence_exists | mutating |
| `register_contradiction_resolution` | pub  | C5 | contradictions resolved_contradictions | resolved_contradictions:A | _advance_state_revision _assert_claim_exists _assert_evidence_exists | C1:_advance_state_revision C1:_assert_claim_exists C1:_assert_evidence_exists | mutating |
| `resolve_disputed_claim_if_ready` | pub  | C5 | claims | claims:A | _advance_state_revision _assert_claim_exists _record_claim_lifecycle_transition active_contradictions_for_claim | C1:_advance_state_revision C1:_assert_claim_exists C6:_record_claim_lifecycle_transition | mutating |
| `resolved_contradictions_for_claim` | pub  | C5 | resolved_contradictions | — | _assert_claim_exists | C1:_assert_claim_exists | read-only |
| `_record_claim_lifecycle_transition` | priv | C6 | claim_lifecycle_events lifecycle_seq | claim_lifecycle_events:A lifecycle_seq:W | — | — | mutating |
...
| `register_rule` | pub  | C7 | rule_definitions rule_stats | rule_definitions:A rule_stats:A | _advance_state_revision | C1:_advance_state_revision | mutating |
| `update_rule_stats` | pub  | C7 | rule_stats | rule_stats:A | _advance_state_revision _assert_rule_stats_pair_exists | C1:_advance_state_revision C1:_assert_rule_stats_pair_exists | mutating |
...
C2<->C5 direct calls                 0  (proves C2/C5 independence for separate 3B PRs)
cross-cluster WRITE ownership         stores written by >1 cluster = ['_claims']  (operational; excludes C10 bulk _install)
module-function state-port width      19 stores + 12 private methods (transitive closure)
  directly called by non-infra        11 methods; +1 transitive helper (_storage_for_kind via _id_exists) = 12
  port stores  (19): ['claim_gap_refs', 'claim_lifecycle_events', 'claims', 'contradictions', 'entities', 'evidences', 'gap_dedup_index', 'gap_resolutions', 'gaps', 'hint_evidence_types', 'lifecycle_seq', 'next_id', 'observations', 'relations', 'resolved_contradictions', 'rule_definitions', 'rule_stats', 'state_identity_token', 'state_revision']
  port methods (12): ['_advance_state_revision', '_allocate_id', '_assert_claim_exists', '_assert_entity_exists', '_assert_evidence_exists', '_assert_gap_exists', '_assert_rule_pair_exists', '_assert_rule_stats_pair_exists', '_id_exists', '_record_claim_lifecycle_transition', '_storage_for_kind', 'state_identity']
```

These values reproduce the ADR's headline claims: DAG (0 SCCs); single
revision authority (`_advance_state_revision`, 20 callers) and single ID
authority (`_allocate_id`, 6 callers); `_claims` the only store written by
>1 cluster (operational, excluding C10 bulk `_install`); C2↔C5 direct calls = 0;
module-function port width = 19 stores + 12 private methods (transitive closure;
11 called directly by non-infra clusters). Both ≈ the entire private surface.
